import { ref as S, onMounted as O, openBlock as H, createElementBlock as U, createElementVNode as M, withModifiers as x, toDisplayString as F, unref as Q } from "/view-ui/libs/vue.js";
import { _ as G } from "./default2.js";
...
}(), X = document.documentElement.tagName.toLowerCase() === "svg", b = X ? N : K() ? function() {
  function t() {
...
function Y(t, e) {
  for (var a = 1, r = J(t), i = 0, n = T.length; i <= n; i++) {
    var o = 0;
    switch (e) {
      case D.L:
        o = T[i][0];
...
B = function(t, e) {
  ...
  typeof t == "string" && (t = document.getElementById(t)), this._htOption.useSVG && (b = N), this._android = y(), this._el = t, this._oQRCode = null, this._oDrawing = new b(this._el, this._htOption), this._htOption.text && this.makeCode(this._htOption.text);
};
B.prototype.makeCode = function(t) {
  this._oQRCode = new E(Y(t, this._htOption.correctLevel), this._htOption.correctLevel), this._oQRCode.addData(t), this._oQRCode.make(), this._oDrawing.draw(this._oQRCode), this.makeImage();
};
...
B.CorrectLevel = D;
var V = B;
const W = `.qrcode[data-v-14463767]{position:fixed;left:0;right:0;top:0;bottom:0;z-index:10000000;background:linear-gradient(130deg,rgba(165,165,165,.7),rgba(56,56,56,.8));display:flex;flex-direction:column;justify-content:center;align-items:center}.qrcode .link[data-v-14463767]{margin-top:15px;padding:10px 12px;background-color:#eee;border-radius:20px}.qrcode[data-v-14463767] img{width:200px;height:200px;border-radius:6px;padding:10px;background-color:#fff}
`, z = {
  __name: "qrcode",
  props: { href: String },
  emits: ["close"],
  setup(t, { emit: e }) {
    const { href: a } = t, r = a || location.href, i = S();
    return O(() => {
      new V(i.value, {
        text: r,
        width: 200,
...
}, et = /* @__PURE__ */ G(z, [["styles", [W]], ["__scopeId", "data-v-14463767"]]);
export {
  et as default
};
